...
import com.yqy.kotlin.utils.JsonUtil;
...
public class ProgressSubscriber<T> extends Subscriber<T> implements ProgressCancelListener {
    ...
    public void onError(Throwable e) {
        ...
        if(e.getMessage().indexOf("errorCode") != -1){
            Map<String,String> errorMap = JsonUtil.INSTANCE.jsonToMap1(e.getMessage());
...
//                ((AbstractActivity) mContext).finish();
        ...
        if(L.INSTANCE.isShow()) {
            e.printStackTrace();
            L.INSTANCE.e("error",e.getMessage());
        }
    }
